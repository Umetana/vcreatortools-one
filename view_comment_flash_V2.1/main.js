...
createApp({
  setup() {
    const comments = ref([]);
    // コメントごとの抽選結果は設定プレビューでも保持する。
    const randomEffects = ['glow', 'shimmer', 'glint', 'sweep', 'aurora', 'trace'];
    const effectFor = (comment) => C.GIFT_EFFECT === 'random'
      ? comment.randomEffect : C.GIFT_EFFECT;
    ...
    const applyPreviewConfig = (nextConfig) => {
      ...
      comments.value = comments.value.map((current) => {
        ...
        return {
          ...
          randomEffect: current.randomEffect,
    ...
    const addComment = (raw) => {
      ...
      const newCmt = {
        ...
        randomEffect: (comment.isSupport || comment.isMembership)
          ? randomEffects[Math.floor(Math.random() * randomEffects.length)] : 'none',
        timestamp: Date.now()
      };
    ...
    onMounted(() => {
      ...
      OneSDK.ready().then(() => {
        OneSDK.connect();
        console.log(`View Comment Flash V2.1: Ready (SDK: ${VCT_SDK.VERSION}, Stack: ${C.STACK_DIRECTION || 'up'})`);
      });
    });
