(function () {
  ...
  const updateSource = () => {
    ...
    const profile = runtime.profileId === 'default' ? '標準' : runtime.profileId;
    sourceText.textContent = `${labels[runtime.source] || runtime.source} ｜ プロファイル: ${profile}${runtime.profileWarning ? ' ｜ ' + runtime.profileWarning : ''}`;
  };
  ...
  const updateEffectVisibility = () => {
    controls.querySelectorAll('[data-effect]').forEach(group => { group.hidden = group.dataset.effect !== draft.COMMENT_EFFECT; });
    controls.querySelectorAll('[data-display]').forEach(group => { group.hidden = group.dataset.display !== draft.DISPLAY_MODE; });
  };
  const render = () => {
    ...
    for (const section of Object.values(schema)) {
      ...
      if (section.effect) group.dataset.effect = section.effect;
      if (section.display) group.dataset.display = section.display;
      group.hidden = (!!section.effect && section.effect !== draft.COMMENT_EFFECT) || (!!section.display && section.display !== draft.DISPLAY_MODE);
