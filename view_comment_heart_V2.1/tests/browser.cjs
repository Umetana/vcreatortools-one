...
(async () => {
  ...
  try {
    ...
    const urls = [
      `${origin}/view_comment_heart_V2.1/index.html?settings=1`,
    ...
    for (const url of urls) {
      ...
      assert.equal(await page.locator('.cmt__heart').count(), 20);
      ...
      await page.locator('[data-config-key="HEART_COUNT"]').fill('2');
      await page.waitForFunction(() => document.querySelectorAll('.cmt__heart').length === 8);
      await page.locator('[data-config-key="HEART_MODE"]').selectOption('off');
      await page.waitForFunction(() => document.querySelectorAll('.cmt__heart').length === 0);
      await page.locator('.vct-settings-close').click();
      await page.waitForFunction(() => document.querySelectorAll('.cmt__heart').length === 20);

      await page.locator('#vct-settings-launcher').click({ force: true });
      await page.locator('[data-config-key="HEART_COUNT"]').fill('3');
      await page.getByRole('button', { name: '保存して再読み込み', exact: true }).click();
      await page.waitForFunction(() => window.CONFIG?.HEART_COUNT === 3);
      await page.locator('#vct-settings-root:not([hidden])').waitFor();
      await page.getByRole('button', { name: 'ローカル設定を削除', exact: true }).click();
      await page.waitForFunction(() => window.CONFIG?.HEART_COUNT === 5);
    ...
    console.log('PASS: HTTP/file起動、SDK V2ギフト分類、ハート演出、設定プレビュー・保存・削除、翻訳、重複、クリア');
  } finally {
