...
createApp({
  setup() {
    ...
    const buildHearts = (comment) => {
      ...
      if (mode !== 'always' && !(mode === 'gift' && comment.isSpecial)) return [];

      const count = Math.max(0, Number(C.HEART_COUNT || 5));
      const sizeMin = Number(C.HEART_SIZE_MIN || 14);
      const sizeMax = Math.max(sizeMin, Number(C.HEART_SIZE_MAX || 24));
      const durationMin = Number(C.HEART_DURATION_MIN || 2);
      const durationMax = Math.max(durationMin, Number(C.HEART_DURATION_MAX || durationMin));
      const delayMax = Math.max(0, Number(C.HEART_DELAY_MAX || 0));
    ...
    onMounted(() => {
      ...
      OneSDK.ready().then(() => {
        OneSDK.connect();
        console.log(`View Comment Heart V2.1: Ready (SDK: ${VCT_SDK.VERSION}, Stack: ${C.STACK_DIRECTION || 'up'})`);
      });
    });
